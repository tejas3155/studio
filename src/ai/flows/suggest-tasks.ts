...
/**
 * @fileOverview An AI agent that suggests tasks based on the content the user is working on.
 *
 * - suggestTasks - A function that suggests tasks based on the content.
...
const SuggestTasksInputSchema = z.object({
  content: z.string().describe('The content the user is working on.'),
});
export type SuggestTasksInput = z.infer<typeof SuggestTasksInputSchema>;

const SuggestTasksOutputSchema = z.object({
  tasks: z.array(z.string()).describe('A list of suggested tasks.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant that helps users stay organized by suggesting tasks based on the content they are working on.

  Suggest a list of tasks that the user should do based on the content below.
  Content: {{{content}}}
  Tasks:`,
});
